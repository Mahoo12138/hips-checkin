...
<div class="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 shadow-lg select-none">
	<div class="flex items-center justify-between mb-4 px-2">
		<button
			onclick={handlePrevMonth}
			class="p-2 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-cyan-400 transition"
		>
	...
	<div class="grid grid-cols-7 gap-1 text-center mb-2">
		{#each ['日', '一', '二', '三', '四', '五', '六'] as day}
			<div class="text-slate-500 text-xs font-medium py-2">{day}</div>
		{/each}
	</div>
	...
	<div class="grid grid-cols-7 gap-1">
		{#each calendarDays as day}
			{#if day === null}
				<div class="aspect-[4/5] md:aspect-square"></div>
			{:else}
				...
				<button
					...
					disabled={isDisabled}
					class="relative aspect-[4/5] md:aspect-square flex flex-col items-center justify-start pt-2 rounded-xl transition-all duration-200 border
						{isSelected ? 'ring-2 ring-cyan-500 z-10' : 'hover:bg-slate-800/80'}
						{isCurrentDay && !isSelected ? 'ring-1 ring-cyan-500/30' : ''}
						{getStatusBg(status)}
						{isDisabled ? 'opacity-50 cursor-not-allowed grayscale' : 'cursor-pointer'}
					"
				>
					<span class="text-sm font-medium {isSelected ? 'text-cyan-400' : 'text-slate-300'}">
						{day}
					</span>
					
					<div class="mt-1 px-1 w-full flex flex-col items-center gap-0.5 overflow-hidden">
						{#if projName}
							<span class="text-[10px] leading-tight text-center line-clamp-2 text-slate-400 w-full px-0.5">
								{projName}
							</span>
						{/if}
						
						<span class="text-[10px] font-bold scale-90 {getStatusColor(status)}">
							{status === 'Empty' ? '' : status}
						</span>
					</div>
				</button>
